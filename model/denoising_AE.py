...
class AutoEncoder(nn.Module):
    def __init__(self, input_dims, code_dims, use_cuda=True):
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(code_dims, 64, kernel_size=(8,8), stride=1), # b x 64 x 14 x 14
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(64, input_dims, kernel_size=(15,15), stride=1), # b x input_dims x 28 x 28
            nn.Sigmoid()
        )
        if use_cuda:
            self.encoder.cuda()
